...
function FilteredStudents() {
    const{filterData,setFilterData,studentsData,setStudentsData} = useContext(store)
  ...
  return (<>
      ...
      <table className="studentTable">
                ...
                { currentItems.map((student,index)=>{
                    ...
                    <td>{student.email}</td>
                    <td>{student.class}</td>
                    <td className='btnTD'><button className='viewBtn' onClick={()=>{navigate(`/oneStudentDataView/${student.id}`)}}>View</button><button className='deleteBtn' onClick={()=>{deleteStudent(student.id)}}>Delete</button><button className='editBtn' onClick={()=>{navigate(`/editDtudentData/${student.id}`)}}>Edit</button></td>
                    </tr>
                    </>)
                })}
                </table>
